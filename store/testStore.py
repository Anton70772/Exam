...
def save_order():
    ...
    qr_code_filename = f"qrcode_{order_number}.png"

    generate_qr_code(pickup_code, qr_code_filename)
    ...
    img = PlatypusImage(qr_code_filename, width=100, height=100)
    order_data.append(Spacer(0, 20))
    order_data.append(Paragraph(f"Код получения: {pickup_code}", styles['Normal']))
    order_data.append(Paragraph(f"Срок доставки: {delivery_time}", styles['Normal']))
    order_data.append(img)  # Adding QR code
